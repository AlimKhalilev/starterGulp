...
# [starterGulp v1.2](https://alimkhalilev.github.io/starterGulp/starterGulp/)
...
> + Используется компонентный подход верстки, а именно методология **БЭМ** в ее нотации **Two Dashes**, где модификатор отделяется двумя дефисами;
...
> + *SVG* иконки вставляются в одноименную папку, а далее автоматически генерируются в спрайт. *К примеру, если спрайт называется **arrow**, его можно использовать как:*
``` HTML
...
Не обязательно убирать у ссылки атрибут **href**, так как там все равно сработает **e.preventDefault()**
...
там прописываются стили в виде **.class_name**, в блоках использовать **.section-block-content**
